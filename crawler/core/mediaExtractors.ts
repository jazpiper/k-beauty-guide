...
const CONTAINER_PATTERN_MEDIA =
  /<(div|section|article|main|ul)\b([^>]*)>([\s\S]*?)<\/\1>/gi;

function extractContainerById(html: string, id: string): string[] {
  if (!html.includes(id)) return [];
  const containers: string[] = [];
  for (const match of html.matchAll(CONTAINER_PATTERN_MEDIA)) {
    const attrs = match[2] ?? "";
    if (!attrs.includes(id)) continue;
    const elementId = getAttrValue(attrs, "id");
    if (elementId === id) containers.push(match[3] ?? "");
  }
  return containers;
}
