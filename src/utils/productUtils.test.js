...
describe("productUtils", () => {
  ...
  describe("normalizeImage", () => {
    ...
    it("returns string image url directly", () => {
      expect(normalizeImage("https://img.com/a.jpg")).toBe(
        "https://img.com/a.jpg",
      );
    });

    it("extracts url from image object properties", () => {
      expect(normalizeImage({ url: "https://img.com/url.jpg" })).toBe(
        "https://img.com/url.jpg",
      );
      expect(normalizeImage({ src: "https://img.com/src.jpg" })).toBe(
        "https://img.com/src.jpg",
      );
      expect(normalizeImage({ imageUrl: "https://img.com/imageUrl.jpg" })).toBe(
        "https://img.com/imageUrl.jpg",
      );
      expect(
  ...
  describe("getHighestSeverity", () => {
    ...
    it("computes highest severity from ingredients using safety if highestSeverity is missing", () => {
      ...
      expect(getHighestSeverity({}, [], ingredients)).toBe(
        "avoid_if_sensitive",
      );
    });
  ...
  describe("getRecommendedProducts", () => {
    it("returns explicit recommended products from detail data up to 3", () => {
      const explicitRecs = [
        { id: "p1", name: "Rec 1" },
        { id: "p2", name: "Rec 2" },
        { id: "p3", name: "Rec 3" },
      ...
      const result = getRecommendedProducts(null, explicitRecs);
      expect(result).toHaveLength(3);
      expect(result[0].name).toBe("Rec 1");
    });

    it("returns empty array if no product and no detail recommendations", () => {
      expect(getRecommendedProducts(null, [])).toEqual([]);
    });

    it("returns sorted fallback recommendations based on category, skin, and safety signals", () => {
      const product = {
        slug: "cosrx-snail-96-mucin-power-essence",
        category: "Serum",
        skin: "Sensitive/Dry",
      };
      const result = getRecommendedProducts(product, [], "caution", 1);
      expect(result.length).toBeGreaterThan(0);
      expect(result.length).toBeLessThanOrEqual(3);
      expect(result.every((item) => item.slug !== product.slug)).toBe(true);
    });
  });
});
