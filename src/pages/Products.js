...
export default function Products() {
  ...
  useEffect(() => {
    ...
    setActiveCategory(
      productCategories.includes(categoryParam) ? categoryParam : "All",
    );
  }, [location.search]);
  ...
  const filtered = products
    .filter((p) => {
      ...
      const searchHaystack = [p.name, p.brand, p.category, p.skin, p.tag]
        .join(" ")
        .toLowerCase();
  ...
  return (
    <div className="products-page">
      <div className="products-hero">
        <h1>
          🛍️ K-Beauty <span className="pink">Products</span>
        </h1>
          ...
          <div className="filter-group">
            ...
            {skinTypes.map((s) => (
              <button
                ...
                className={`filter-btn ${activeSkin === s ? "active" : ""}`}
              >
                {s}
              </button>
            ))}
                  ...
                  <div className="product-rating">
                    {p.rating ? (
                      <>
                        ...
                        <span>
                          {p.rating} ({(p.reviews ?? 0).toLocaleString()})
                        </span>
                      </>
                    ) : (
                      <span>
                        {p.safetyFlagCount > 0
                          ? `${p.safetyFlagCount} ingredient note${p.safetyFlagCount > 1 ? "s" : ""}`
                          : "No safety flags yet"}
                      </span>
                    )}
                  </div>
